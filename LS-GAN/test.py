...
if True:
    # folder dataset
    dataset = dset.ImageFolder(root='/content/celeba',
...
with torch.cuda.device(opt.gpu):
    ...
    # -------- Run on GPU --------
    fix_noise = torch.rand(100,nz,1,1)
    if opt.cuda:
        netD.cuda()
        netG.cuda()
    ...
    for epoch in range(opt.niter):
        ...
        while i < len(dataloader):
            ...
            # Loss for D.
            errD = LeakyReLU(outputR - outputF + pdist).mean()+gp
            errD.backward()
            ...
            if (i%20==0):
                print('G_loss {} D_loss {}'.format(errG,errD))
                vis_img = netG(fix_noise)
